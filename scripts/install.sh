...
function saveProfile(){
	...
	if [ -s userChrome.css ]; then
		...
		_sed 's/@import "arc-gnome-theme.*.//g' userChrome.css | _sed '/^\s*$/d' > tmpfile && mv tmpfile userChrome.css
		echo >> userChrome.css
	else
		echo >> userChrome.css
	fi
	...
	if ! cat userChrome.css | grep "arc-gnome-theme" > /dev/null; then
	...
	if [ -s userContent.css ]; then
		# Remove older theme imports
		_sed 's/@import "arc-gnome-theme.*.//g' userContent.css | _sed '/^\s*$/d' > tmpfile1 && mv tmpfile1 userContent.css
	...
	_sed -i '1s/^/@import "arc-gnome-theme\/userContent.css";\n/' userContent.css

	# Check if the import was set correctly (userContent)
	if ! cat userContent.css | grep "arc-gnome-theme" > /dev/null; then
	...
	while IFS= read -r line; do
		theme_prefs+=("$line")
	done < <(grep "user_pref" chrome/arc-gnome-theme/configuration/user.js)

	theme_prefs_unvalued=()
	while IFS= read -r line; do
		theme_prefs_unvalued+=("$line")
	done < <(grep "user_pref" chrome/arc-gnome-theme/configuration/user.js | cut -d'"' -f 2)
