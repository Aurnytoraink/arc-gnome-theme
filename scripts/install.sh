...
function saveProfile(){
	...
	if [ -s userChrome.css ]; then
		...
		sed 's/@import "arc-gnome-theme.*.//g' userChrome.css | sed '/^\s*$/d' > tmpfile && mv tmpfile userChrome.css
		echo >> userChrome.css
	else
		echo >> userChrome.css
	fi
	...
	if [ "$THEME" = "DEFAULT" ]; then
		...
		echo "@import \"arc-gnome-theme\/theme/colors/light-$THEME.css\";" >> userChrome.css
		echo "@import \"arc-gnome-theme\/theme/colors/dark-$THEME.css\";" >> userChrome.css
	fi

	# Create single-line user content CSS files if non-existent or empty.
	if [ -s userContent.css ]; then
		# Remove older theme imports
		sed 's/@import "arc-gnome-theme.*.//g' userContent.css | sed '/^\s*$/d' > tmpfile1 && mv tmpfile1 userContent.css
	...
	if [ "$THEME" = "DEFAULT" ]; then
		...
		echo "@import \"arc-gnome-theme\/theme/colors/light-$THEME.css\";" >> userContent.css
		echo "@import \"arc-gnome-theme\/theme/colors/dark-$THEME.css\";" >> userContent.css
	fi
	...
	mapfile -t theme_prefs < <( grep "user_pref" chrome/arc-gnome-theme/configuration/user.js )
	mapfile -t theme_prefs_unvalued < <( grep "user_pref" chrome/arc-gnome-theme/configuration/user.js|cut -d'"' -f 2 )
	if [ ! -f "user.js" ]; then
		mv chrome/arc-gnome-theme/configuration/user.js .
	else
